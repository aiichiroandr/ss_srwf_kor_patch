...
export const PATCH_NOTES = Object.freeze({
  ...
  "srwf-f-20260823-v0-3": release(
    "v0.3",
    "배포 전 전수 검사에서 시나리오 7,145행·전투 5,989행·격추 166행 모두 화면 폭 초과가 없음을 확인했습니다. 이 판은 엔딩 크레딧을 일본어 원본으로 되돌렸습니다 — 한글 크레딧 영상은 공개 패치 형식의 record 상한을 넘겨 담을 수 없었고, 다음 판에서 형식을 넓혀 다시 넣습니다.",
    [],
  ),
  "srwf-final-20260814-v0-1": release(
    "v0.1",
    "지금은 설치를 권하지 않습니다. 실제로 플레이하실 생각이라면 이 판은 건너뛰고 다음 판을 기다려 주세요. F 완결편은 F보다 검수가 한참 덜 됐습니다 — 콜드부트와 데모 전투 경로의 한글 표시만 확인했을 뿐, 장시간 실플레이·전 경로 완주·줄바꿈 전수 검수를 아직 하지 않았습니다. 구성은 시나리오 대사(SCEDATA)·전투 메시지(BMESS)·화자명 풀(TSR)·가라오케 자막(KARAOKE)의 내부 r013 작업본을 정품 Rev. A 전체 디스크 기반으로 조립한 것입니다.",
    [
      item({
