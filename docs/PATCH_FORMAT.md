...
7. 사용자가 확인한 원본의 상위 폴더에 기존 파일과 겹치지 않는 새 출력 이름을
   만들고, 원본과 결과의 모든 검사가 끝난 경우에만 저장을 확정합니다. 어느
   하나라도 다르면 임시 출력을 중단합니다.
...
동시에 추가해야 합니다. 결과가 고정 원본보다 커지는 경우에만 쓰는 v2
(`SRWFKP2\0`)는 [PATCH_FORMAT_V2.md](PATCH_FORMAT_V2.md)에 있으며, v1의 규칙과
적용기는 v2 추가로 바뀌지 않습니다.
